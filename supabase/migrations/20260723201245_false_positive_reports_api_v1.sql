-- The table stays private by grants and RLS while remaining reachable by the
-- server-side Edge Function through Supabase's supported Data API.
drop table if exists private.false_positive_reports;

create table public.false_positive_reports (
...
revoke all on public.false_positive_reports from public, anon, authenticated;
grant select, insert on public.false_positive_reports to service_role;

create index false_positive_reports_device_created_idx
  on public.false_positive_reports (device_id_hash, created_at desc);

create index false_positive_reports_fingerprint_idx
  on public.false_positive_reports (rule_fingerprint, created_at desc);
