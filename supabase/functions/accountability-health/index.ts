...
// Device check-ins authenticate with a private device credential. Scheduled
// delivery authenticates with ACCOUNTABILITY_SCHEDULER_SECRET.
const corsHeaders = {
...
Deno.serve(async (request) => {
  ...
  if (request.headers.get("x-accountability-scheduler-secret")) {
    return dispatchStaleProtectionAlerts(request, admin);
  }

  const checkin = await readCheckin(request);
  if (!checkin) return json({ error: "invalid_checkin" }, 400);
  const { data, error } = await admin.rpc("record_accountability_protection_health_checkin", {
  ...
  if (error) return json({ error: error.code === "28000" ? "invalid_device_credential" : "checkin_rejected" }, error.code === "28000" ? 401 : 400);
  return json({ accepted: data?.[0]?.accepted === true, duplicate: data?.[0]?.duplicate === true }, 202);
});

async function dispatchStaleProtectionAlerts(request: Request, admin: ReturnType<typeof createClient>) {
  const expected = Deno.env.get("ACCOUNTABILITY_SCHEDULER_SECRET");
  if (!expected || request.headers.get("x-accountability-scheduler-secret") !== expected) {
    return json({ error: "scheduler_unauthorized" }, 401);
  }
  const { data, error } = await admin.rpc("queue_accountability_protection_health_alerts");
  if (error) return json({ error: "queue_failed" }, 500);

  const rows = (data ?? []) as Array<{ alert_id: string; guardian_user_id: string }>;
  let emailSent = 0;
  for (const row of rows) {
    const { data: userData } = await admin.auth.admin.getUserById(row.guardian_user_id);
    const email = userData.user?.email;
    const delivered = Boolean(email && await sendHealthEmail(email));
    if (delivered) {
      emailSent += 1;
      await admin.rpc("complete_accountability_protection_health_alert", {
...
async function readCheckin(request: Request) {
  try {
    ...
    return value as { accessibilityEnabled: boolean; deviceId: string; idempotencyKey: string; secret: string; vpnEnabled: boolean };
  } catch {
    return null;
  }
}

async function sendHealthEmail(to: string) {
