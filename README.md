...
| --- | --- |
| Versión de la app | `1.3.23` |
| Android | `versionCode 50`; beta técnica con protección nativa mediante VPN local, Accesibilidad e interrupción |
| iOS | `build 14`; candidata técnica con Family Controls, Managed Settings, Device Activity y extensiones Shield |
...
La versión `1.3.23 (build 14)` corrige la puerta de calidad del simulador para desplazar la pantalla hasta las capacidades nativas antes de validarlas. Conserva la corrección de la acción del Shield para el SDK iOS actual, el alta y confirmación del PIN, autorización nativa de Family Controls, selección privada de aplicaciones/categorías/sitios, navegación protegida e interfaz localizada. Todavía debe completar el recorrido de aceptación en un iPhone físico antes de considerarse lista para distribución pública.
...
El artifact contiene solamente `Clean4Jesus.ipa`. Para la versión actual, el nombre esperado es `clean4jesus-ios-ipa-v1.3.23-build-14`. Cada IPA nueva debe incrementar `expo.ios.buildNumber`; cambiar únicamente el código sin aumentar el build puede hacer que se vuelva a instalar una versión anterior o indistinguible.
