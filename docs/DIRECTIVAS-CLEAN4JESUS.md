...
- Un despliegue posterior puede sobrescribir la landing aunque GitHub esté correcto: `npm run landing:deploy` ejecuta el generador retirado y vuelve a crear la página antigua. Para la landing vigente, usar solo `npx wrangler deploy --config wrangler.landing.jsonc` desde el árbol que contiene la landing autocontenida.
...
- Checkpoint físico iOS `1.3.25 (build 16)` del 15 de agosto de 2026: Family Controls, selección privada, límite diario y Shield funcionaron en iPhone. Conservar el tag `checkpoint/ios-v1.3.25-build-16-estable` como rollback antes de ampliar los límites.
- Desde `1.3.26 (build 17)`, iOS usa un `DeviceActivityEvent` independiente por `ApplicationToken`; el uso de una app nunca debe consumir el tiempo de otra. Los tokens, nombres e iconos permanecen locales y opacos, y solo se representan mediante `Label(applicationToken)` dentro de SwiftUI nativo.
- Desde `1.3.27 (build 18)`, iOS no pide PIN al abrir Clean4Jesus. La primera configuración explícita de límites no pide PIN; al guardarla, se crea el PIN de guardianía. Cualquier cambio posterior de tiempos o selección exige ese PIN. No usar el PIN como inicio de sesión ni para bloquear Palabra, Comunidad o Perfil.
- La pantalla Shield de iOS está limitada por Apple a icono, colores, título, subtítulo y acciones. La identidad aprobada usa azul `#071F52`, dorado, escudo con cruz, título de límite alcanzado y un único CTA de cierre. No prometer un layout idéntico al de Android ni una navegación que iOS 16-18 no permite.
- El playbook vigente de identidad es `docs/BRAND-PLAYBOOK.md`. Todo Shield iOS debe cargar `Clean4JesusOfficialMark` desde el activo oficial y aplicar azul marino, blanco y dorado conforme a ese documento; no usar una reconstrucción circular del logo.
...
- Nu, bancos y billeteras no se inspeccionan ni se bloquean por paquete. La lista de paquetes de Accesibilidad no puede incluir bancos ni billeteras. Accesibilidad sigue siendo un requisito activo del onboarding Android: nunca se declara el Refugio preparado basándose sólo en una marca guardada, y nunca se apaga el servicio al terminar esa configuración. Cualquier futura política de compatibilidad bancaria debe evaluarse por separado y no puede degradar el flujo de protección ni prometer una reactivación automática que Android no permita.
