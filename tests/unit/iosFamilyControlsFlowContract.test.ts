...
describe("Family Controls activation flow", () => {
  it("selects Family Activity items before configuring Managed Settings", () => {
    ...
    const configure = source.indexOf("configureProtection({ blockCategories");
  ...
  it("does not reference a missing generated JavaScript native module", () => {
    const source = readFileSync(join(process.cwd(), "modules/clean4jesus-ios-protection/index.js"), "utf8");
    expect(source).toContain("requireNativeModule('Clean4JesusIosProtectionModule')");
