...
describe("Phase 1 protection contracts", () => {
  ...
  it("commits local PIN and interruption settings only after Android accepts them", () => {
    ...
    const customizationSource = read("src/features/interruption/interruptionCustomizationService.ts");
    expect(pinSource.indexOf("syncPinToNative(pinHash)")).toBeLessThan(pinSource.indexOf("setSecureItem(storageKeys.pin"));
    expect(customizationSource.indexOf("syncCustomizationToNative(normalized)")).toBeLessThan(customizationSource.indexOf("setJson(storageKeys.interruptionCustomization, normalized)"));
  });
  ...
  it("keeps the approved interruption hierarchy with reason before rescue and guardian actions", () => {
    ...
    expect(source.indexOf("content.addView(reasonCard)")).toBeLessThan(source.indexOf("content.addView(rescueActionCard)"));
    expect(source.indexOf("content.addView(reasonCard)")).toBeLessThan(source.indexOf("content.addView(unlockCard)"));
  ...
  it("clears native accountability credentials whenever the local session signs out", () => {
    ...
    const accountabilitySource = read("src/features/accountability/accountabilityService.ts");

    expect(authSource).toContain("await clearAccountabilityDevice()");
    expect(providerSource).toContain("clearAccountabilityDevice");
    expect(accountabilitySource).toContain("export async function clearAccountabilityDevice");
    expect(accountabilitySource.indexOf("nativeModule?.clearAccountabilityDevice")).toBeLessThan(
